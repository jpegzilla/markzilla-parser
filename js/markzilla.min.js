...
d=0;d<el.length;d++)line=elements[b],lineLength=line.length,elPosition=elements[b].indexOf(el[d]),currentLink=el[d],linkText=currentLink.match(k)[0].replace(/(\[a\s|\])/gi,""),linkAdd=currentLink.match(l)[0].replace(/(\(|\))/gi,""),elToLast=currentLink.length+elPosition,finalLine=elements[b].substr(0,elPosition)+('<a target="_blank" href="'+linkAdd+'">'+linkText+"</a>")+elements[b].substr(elToLast,lineLength),elements[b]=finalLine}for(b=0;b<elements.length;b++)elements[b]=elements[b].replace(g,"[");
for(b=0;b<elements.length;b++)element=elements[b],index=b,c={},elements[b]=(c[index]=element,c)}),elements;console.warn("no file path specified.");return!1},decon:function(a){},insertPost:function(a){if(Object.keys(a)[0]){var b=window.performance.now(),e=Object.values(a)[0].title,c=Object.keys(a)[0];a=Object.values(a)[0].body;c=parseDateString(c);$(".mkzTitle").html(e);$(".mkzDate").text(c.y+" "+c.m+" "+c.d+" ");for(e=0;e<a.length;e++)a[e]&&$(".mkzBody").append(a[e]);e=window.performance.now();return"completed task in "+
.001*Math.floor(e-b)+" seconds."}console.warn("no postObject defined");return!1},saveToPost:function(a){if(0!==a.length){articleDate=yyyymmdd().yyyymmdd;articleTitle=Object.values(a[0])[0];var b={};postObject=(b[articleDate]={title:articleTitle,body:[]},b);for(b=1;b<a.length;b++)postObject[articleDate].body[b]=Object.values(a[b])[0];return postObject}console.warn("no json to save.");return!1}};
